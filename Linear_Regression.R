...
library(readr)
wc_at <- read.csv("file:///D:/ExcelR/Linear Regression/wc-at.csv")
View(wc_at)
...
logpol <- predict(reg2degree, interval = "confidence")
expy <- exp(logpol)
...
tidy_lmfit <- tidy(model_5)
tidy_lmfit
write.csv(final2,"air_group.csv", row.names = TRUE)
